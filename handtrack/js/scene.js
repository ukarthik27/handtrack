...
var material = new THREE.MeshBasicMaterial( {color: 0xffff00,wireframe:true,wireframeLinewidth:2} );
...
function runDetection() {
  ...
  model.detect(video).then(predictions => {
    ...
    if (predictions.length > 0) {
      console.log(" has some predictions");
...
function moveTheBox(value) {
  console.log("move the box called");
  sphere.position.x = ((window.innerWidth * value.x) / window.innerWidth) * 5;
  sphere.position.y = -((window.innerHeight * value.y) / window.innerHeight) * 5;
  renderer.render(scene, camera);
}
